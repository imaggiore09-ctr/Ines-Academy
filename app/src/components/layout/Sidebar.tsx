...
export function Sidebar() {
  ...
  const userName = user === 'ines' ? 'Ines' : user === 'cristina' ? 'Cristina' : '';
  const userFull = user === 'ines' ? 'Ines de Cueto' : user === 'cristina' ? 'Cristina de Cueto' : '';
  ...
  return (
    <aside className="sidebar">
      ...
      <div className="sidebar-foot">
        <button className="user-chip" onClick={logout} title="Cerrar sesion">
          <div className={'avatar' + (isInk ? ' ink' : '')}>{userName[0]}</div>
          <div style={{ textAlign: 'left', flex: 1, minWidth: 0 }}>
            <div style={{ fontSize: 13.5 }}>{userFull}</div>
            <div className="mono" style={{ fontSize: 10, color: 'var(--ink-faint)' }}>{userRole} - Cerrar sesion</div>
          </div>
